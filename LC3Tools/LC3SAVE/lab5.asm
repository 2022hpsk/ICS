            ...
            LD R6,RETSTACK  ;;R6 <- xFE00
            AND R1,R1,0     ;;(R1 <- 0)
            STI R1,BITNUMBER;;MEM[x6003] <- 0
...
SEARCH      ADD R6,R6,#-1   ;;R6 <- R6 - 1
            ...
            ADD R3,R3,#1    ;;R3 <- R3 + 1
            BR  Judge
Continue    BR  Choose1     ;;choose stack1
...
MAPTOBIT    ADD R6,R6,#-1        ;;R6 <- R6 - 1
            ...
            LD  R2,LIST         ;;R2 <- x7000
            AND R5,R5,0         ;;R5 <- 0 (BITNUMBER)
            LDI R4,CONSTNUM     ;;R4 <- MEM[x6002]   (COUNTER :R4)         
            
Nextbit     ADD R2,R2,#1        ;;R2 <- R2 + 1
            LDR R1,R2,0         ;;R1 <- MEM[R2] (counter) (;;R0 <- BIT[R1]:1)
            AND R0,R0,0         ;;R0 <- 0
            ADD R0,R0,#1        ;;R0 <- R0 + 1
Loop        ADD R0,R0,R0        ;;R0 << 1
            ...
            ;;LDI R1,BITNUMBER    ;;R1 <- MEM[x6003] 
            ADD R1,R5,0         ;;R1 <- R5(BITNUMBER)
            NOT R0,R0
            NOT R1,R1
            AND R0,R0,R1
            NOT R0,R0           ;;R0 <- R0 | R1
            
            ADD R5,R0,0         ;;R5(BITNUMBER) <- R0
            ADD R4,R4,#-1
            
            BRp Nextbit
            STI R5,BITNUMBER    ;;MEM[x6003] <- R5
